...
resource "aws_lb" "alb-purina" {
  name            = "ECS-FastAPI-Purina"                     # Name of the ALB
...
resource "aws_lb_listener" "http" {
  load_balancer_arn = aws_lb.alb-purina.arn                  # ARN of the ALB
...
output "IP" {
  value = aws_lb.alb-purina.dns_name
}
...
locals {
  ansible_hosts = templatefile("${path.module}/hosts.tpl", { dns_name = aws_lb.alb-purina.dns_name })
}
